...
`systemd-analyze verify` on (`etc/systemd/system/cuems-gradient-motiond.service`) completed with exit code 0.

**Binary check**: `/usr/bin/gradient-motiond` is executable; `--help` lists `--osc-port`, `CUEMS_GRADIENT_OSC_PORT`, `--midi-port`, and `--log-level`, which matches what the unit passes.

Suggested on-node checks (if this tree is installed or you symlink/copy the unit):
