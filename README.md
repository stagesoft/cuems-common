...
| `cuems-cluster-poweroff` | bash | Orderly cluster power-off, run as the `ExecStop=` of `cuems-cluster-poweroff.service` so it fires during a poweroff transition (power button via logind, or `systemctl poweroff`). Projectors off over PJLink → cluster node(s) off over SSH → wait until really down → hand back to systemd. Runs the installed `cuems-power-bridge`'s power-off stages as a module through `venv_python`, so it duplicates no protocol logic, honours that host's `power-bridge.conf`, and cannot disagree with the bridge's own `POST /shutdown` — both call one implementation. Skips on a reboot, self-excludes this host from the node list, and is idempotent. Gated by `enabled=` in `/etc/cuems/cluster-poweroff.conf` (ships `false`). Replaced the retired `cuems-stop`. Since 1.3.0-23 the stages themselves live in `cuems-power-bridge` (selection on `node_role` + `<adopted>`, one liveness probe, SSH only to nodes that answered), and this script keeps what is its own: the `enabled=` switch, the poweroff-vs-reboot check, the per-stage timeouts, the ordering probe and one `flock` held across both stages. `--force` additionally refuses while a project is playing (`--while-playing` overrides); a poweroff transaction never asks, so the venue can always be powered off mid-show. Verified on hardware from the bridge's ledger (`specs/002-cluster-poweroff-cli/checklists/hardware-verification.md`), not from here. |
...
| `/etc/cuems/network_map.xml` | Cluster topology: one `CuemsNode` element per machine with uuid, mac, name, node_role, ip, port, online fields. Validated against `network_map.xsd` (namespace `https://stagelab.coop/cuems/`). |
| `/etc/cuems/network_map.xsd` | XML Schema for `network_map.xml`. `NodeType` requires uuid, mac, name, node_role, ip, port, and online (BoolType: `True`/`False`). `node_role` is an enumeration (`controller`/`node`/`firstrun`) — feature 007, was free-text `node_type`. |
...
| `/etc/avahi/services/cuems.service` | Active Avahi service group. Announces `_cuems_nodeconf._tcp` on port 9000 and `_cuems_osc._tcp` on port 9090, both carrying `node_role` and `uuid` TXT records. Swapped between the firstrun / controller / node templates by `cuems-config-node` or `cuems-nodeconf`. Not shipped by any package: created by copying a template, and migrated from the retired `node_type=` key on upgrade by `cuems-migrate-avahi-service`. |
| `/usr/share/cuems/cuems.service.{firstrun,controller,node}` | Avahi service group templates. Differ only in the `node_role` TXT record (`firstrun`, `controller`, `node`). Updated by `cuems-config-node write` to embed the correct UUID. |
...
- **Avahi mDNS for node discovery.** The controller announces itself as `controller.local` via Avahi. Service type `_cuems_nodeconf._tcp` on port 9000 carries `node_role` and `uuid` TXT records. Nodes discover the controller purely by mDNS; no static IP configuration is needed beyond the LAN setup.
...
|---|---|---|---|
| `_cuems_nodeconf._tcp` | 9000 | `node_role=controller`, `uuid=<node-uuid>` | Node configuration and cluster membership |
| `_cuems_osc._tcp` | 9090 | `node_role=controller`, `uuid=<node-uuid>` | OSC control protocol |
