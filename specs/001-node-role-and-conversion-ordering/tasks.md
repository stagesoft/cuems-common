...
- [ ] T019 [US1] 🚧 **MERGE GATE (FR-009) — blocking, do not clear early.** Confirm flow 04's counterpart branch in `../cuems-nodeconf` is reviewed and ready, record its branch and commit in this branch's PR description, and agree the simultaneous-merge window. **Target branch: `rc_1`** (fast-forward from this branch). Pin each side with a tag — `flow03-merge-candidate` here, `flow04-merge-candidate` there — so the commits the two gates cite do not move when the gate-closing commits land. Flow 04 is being implemented in parallel; this gate blocks US1's **merge**, not its implementation. Neither half merges alone: a publisher and a listener disagreeing about the key discover nothing, silently
