...
Ships: systemd service/target/path/socket units for both roles + drop-ins for third-party units (Apache2, hostapd, Avahi, rtpmidid); operator tools in `usr/bin/`; internal service helpers in `usr/lib/cuems/bin/`; per-install config + XSD schemas in `etc/cuems/`; Avahi/interface templates in `usr/share/cuems/`; kernel sysctl, JACK, Apache2, DHCP/hostapd, ALSA, Avahi, rsyslog, logrotate, sudoers, tmpfiles, modules-load drop-ins in `etc/`. Does **not** ship `cuems-power-bridge.service`: that unit has come from the `cuems-power-bridge` package itself since its `bb69a9e` (2026-05-29). What this package still ships for the bridge are the two units that DRIVE it — `cuems-cluster-poweroff.service` and `cuems-displays-on.service` — plus their conffile and the poweroff sudoers drop-ins.
